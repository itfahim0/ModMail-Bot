...
function parseArgs(): RotateConfig {
    ...
    if (!oldKeyHex || !HEX_REGEX.test(oldKeyHex)) {
        throw new Error(
            'Old Key must be a valid 32-byte hex string. Provide via --from-key or ENCRYPTION_KEY_OLD.',
        );
    }
    if (!newKeyHex || !HEX_REGEX.test(newKeyHex)) {
        throw new Error(
            'New Key must be a valid 32-byte hex string. Provide via --to-key or ENCRYPTION_KEY_NEW.',
        );
    }
...
interface ModelDelegate {
    ...
    findMany(args: { take: number; skip: number }): Promise<Record<string, unknown>[]>;
    update(args: {
        where: { id: string };
...
async function rotateEncryptedModel(
    ...
    for (let i = 0; i < total; i += config.batchSize) {
        ...
        for (const item of batch) {
            try {
                const ciphertext = item[encryptedFieldName] as string | undefined;
                if (!ciphertext) {
                    processed++;
                    continue;
                }
                ...
                try {
                    ...
                    const looksLikeV1 =
                        ciphertext.startsWith('v1:') && ciphertext.split(':').length === 4;
                    const isExplicitPlaintext = ciphertext.startsWith('otpauth://');

                    if (!looksLikeV1 || isExplicitPlaintext) {
                        logger.warn(
                            `⚠️  Found likely unencrypted data for ${modelName} ${item.id} (${String(item[displayFieldName])}). Encrypting now.`,
                        );
                ...
                if (newCiphertext !== ciphertext) {
                    ...
                    if (!config.dryRun) {
                        ...
                        // Verification
                        const check = await modelDelegate.findUnique({
                            where: { id: String(item.id) },
                        });
                        if (
                            check &&
                            decryptValue(check[encryptedFieldName] as string, config.newKey) ===
                                plaintext
                        ) {
                            verified++;
                        } else {
    ...
    logger.info(
        `✅ ${modelName}: Scanned ${processed}, Updated ${updated}, Verified ${verified}, Failed ${failed}`,
    );
    return failed;
}
...
async function main() {
    try {
        ...
        if (config.oldKey.equals(config.newKey)) {
            logger.info(
                'ℹ️ Old Key matches New Key. This operation will update ciphertext formats to v1.',
            );
        } else {
        ...
        if (totalFailures > 0) {
            logger.error(
                `🚨 Rotation completed with ${totalFailures} failures. Please check logs and intervene manually.`,
            );
            process.exit(1);
        }
