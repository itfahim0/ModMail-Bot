...
export default {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option.setName('link').setDescription('Add an external link').setRequired(false),
        ),
    ...
    async renderDashboard(interaction, isUpdate = false) {
        ...
        if (!data) {
            return interaction.reply({
                content: '❌ Session expired. Please run /dm again.',
                ephemeral: true,
            });
        }
        ...
        const dashboardEmbed = new EmbedBuilder()
            ...
            .setDescription(
                'Configure your direct message below.\n\n**Preview of Message Content:**',
            )
            .setColor(data.color)
        ...
        if (data.message) {
            ...
            dashboardEmbed.addFields({
                name: 'Content',
                value: '*No message set. Click "Set Message" to type it.*',
            });
        }
        ...
        if (
            ...
            data.attachment.contentType.startsWith('image/')
        ) {
    ...
    async handleInteraction(interaction) {
        ...
        if (interaction.customId === 'dm_set_message') {
            ...
            await interaction.followUp({
                content:
                    '✍️ **Please type the DM content in this channel now.**\nType `cancel` to abort.',
                ephemeral: true,
            });
            ...
            collector.on('collect', async (m) => {
                if (m.content.toLowerCase() === 'cancel') {
                    await interaction.followUp({
                        content: '❌ Message setup cancelled.',
                        ephemeral: true,
                    });
                } else {
            ...
            try {
                ...
                await interaction.editReply({
                    content: `✅ DM sent to **${data.targetUser.tag}**!`,
                    embeds: [],
                    components: [],
                });
                dmCache.delete(userId);
                ...
                console.error(error);
                await interaction.editReply({
                    content: `❌ Failed to send DM: ${error.message} (User likely has DMs off)`,
                    ephemeral: true,
                });
            }
        }
