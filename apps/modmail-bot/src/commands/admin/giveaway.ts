...
export default {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option.setName('prize').setDescription('Prize to win').setRequired(true),
        )
        .addStringOption((option) =>
            option.setName('time').setDescription('Duration (e.g., 10m, 1h, 1d)').setRequired(true),
        )
        .addIntegerOption((option) =>
            option.setName('winners').setDescription('Number of winners').setRequired(true),
        )
        .addStringOption((option) =>
            option.setName('message').setDescription('Additional message').setRequired(false),
        ),
    ...
    async execute(interaction) {
        ...
        if (unit === 'm') duration = value * 60 * 1000;
            ...
            return interaction.reply({
                content: '❌ Invalid time format. Use m/h/d (e.g., 10m, 1h)',
                ephemeral: true,
            });
        ...
        setTimeout(async () => {
            const fetchedMsg = await interaction.channel.messages
                .fetch(giveawayMsg.id)
                .catch(() => null);
            ...
            const winnerString = Array.isArray(winners)
            ...
            await fetchedMsg.reply({
                content: `Congratulations ${winnerString}! You won **${prize}**!`,
                embeds: [endEmbed],
            });
        }, duration);
    },
